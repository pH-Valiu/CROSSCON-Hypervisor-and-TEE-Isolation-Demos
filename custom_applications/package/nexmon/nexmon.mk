...
NEXMON_SOURCE = $(KERNEL_VERISON).tar.xz
...
define NEXMON_EXTRACT_CMDS
	tar -xvJf $(@D)/$(NEXMON_KERNEL_VERSION).tar.xz -C $(@D)
endef

define NEXMON_INSTALL_TARGET_CMDS
	$(INSTALL) -D -m 0755 $(@D)/$(NEXMON_KERNEL_VERSION)/nexutil/nexutil $(TARGET_DIR)/usr/local/bin/nexutil
	$(INSTALL) -D -m 0755 $(@D)/$(NEXMON_KERNEL_VERSION)/makecsiparams/makecsiparams $(TARGET_DIR)/usr/local/bin/mcp
	$(INSTALL) -D -m 0755 $(@D)/$(NEXMON_KERNEL_VERSION)/makecsiparams/makecsiparams $(TARGET_DIR)/usr/local/bin/makecsiparams
